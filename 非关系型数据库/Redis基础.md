...
[redis IO多路复用模型](https://blog.csdn.net/Seky_fei/article/details/106677043)
...
|  序号   | type  |  encoding   |
|  ----  | ----  |  ----  |
| 1  | String | （1）int:值为整型，取值[-2^63,2^63-1]<br> （2）embstr: 值不为整型或整型值不在上述范围，长度小于等于44个字节 <br> （3）raw:值不为整型或整型值不在上述范围，且长度超过44(64-16-3-1)字节 <br> 注意:不同的redis版本,embstr与raw的边界值不同|
